...
namespace NinosConValorAPI.Controllers
{
    ...
    public class AssetCategoriesController : Controller
    {
        ...
        public async Task<IActionResult> UpdateAssetCategoryAsync(int assetCategoriesId, [FromBody] AssetCategoryModel assetCategory)
        {            
            try
            {
                return Ok(await _assetCategoriesService.UpdateAssetCategoryAsync(assetCategoriesId, assetCategory));
            }
            catch (NotFoundElementException ex)
            {
                return NotFound(ex.Message);
        ...
        public async Task<ActionResult> DeleteAssetCategoriesAsync(int assetCategoriesId)
        {            
            try
            {
                await _assetCategoriesService.DeleteAssetCategoriesAsync(assetCategoriesId);
                return Ok();
            }
            catch (NotFoundElementException ex)
            {
